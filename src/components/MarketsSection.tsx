...
const MarketsSection = () => {
  const { indices, cryptoAssets, stockAssets } = useMarketData();
  // Manual Fear & Greed index — edit these values by hand
  const fearGreed = {
    crypto: {
      value: 62,                  // 0–100 score
      label: "Greed",             // text you want to show
      updatedAt: "2025-12-03T00:00:00Z", // for the "last updated" text
    },
    stock: {
      value: 55,
      label: "Greed",
  ...
  const cryptoNeedle = getNeedleEndpoint(fearGreed.crypto.value, 100, 100, 55);
  const stockNeedle = getNeedleEndpoint(fearGreed.stock.value, 100, 100, 55);

  // Mock market cap data - will be moved to hook later
  const marketCapRankings = [
    { rank: 1, name: "Gold", symbol: "GOLD", marketCap: "$29.16T", price: "$4,188", change: -0.35, bgColor: "bg-yellow-100" },
    { rank: 2, name: "NVIDIA", symbol: "NVDA", marketCap: "$3.61T", price: "$147.01", change: 3.20, bgColor: "bg-green-100" },
    { rank: 3, name: "Apple", symbol: "AAPL", marketCap: "$3.72T", price: "$243.84", change: 1.45, bgColor: "bg-blue-100" },
    { rank: 4, name: "Microsoft", symbol: "MSFT", marketCap: "$3.18T", price: "$431.27", change: 0.82, bgColor: "bg-purple-100" },
    { rank: 5, name: "Bitcoin", symbol: "BTC", marketCap: "$1.89T", price: "$96,723", change: -2.14, bgColor: "bg-orange-100" },
  ];
  ...
  const getRelativeTime = (isoString: string) => {
    ...
    // If data is older than 10 minutes, show "US MARKET CLOSED"
    if (minutes >= 10) {
        ...
        >
          ...
          <Card className="p-3 md:p-8 bg-card hover:shadow-lg transition-shadow order-1 lg:order-1">
            ...
            <div className="text-center">
              <p className="text-2xl md:text-5xl font-bold">{fearGreed.crypto.value}</p>
              <p className="text-xs md:text-lg text-destructive font-semibold">{fearGreed.crypto.label}</p>
            </div>
          </Card>
          ...
          <Card className="p-3 md:p-8 bg-card hover:shadow-lg transition-shadow order-2 lg:order-3">
            ...
            <div className="text-center">
              <p className="text-2xl md:text-5xl font-bold">{fearGreed.stock.value}</p>
              <p className="text-xs md:text-lg text-destructive font-semibold">{fearGreed.stock.label}</p>
            </div>
          </Card>
      ...
      <div>
        ...
        <p className="text-lg md:text-2xl font-bold">
          {index.price.toLocaleString()}
            ...
            {/* Bitcoin card */}
            {cryptoAssets[0] && (
              <Card className="p-3 md:p-4 bg-card hover:shadow-lg transition-shadow border-2">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-xs md:text-sm font-semibold text-muted-foreground">Bitcoin (BTC)</h4>
                    <p className="text-lg md:text-2xl font-bold">${(cryptoAssets[0].price ?? 0).toLocaleString()}</p>
                  </div>
                  <div className="text-right space-y-1">
                    <p className="text-xs text-muted-foreground">24h change</p>
                    <div className={`flex items-center justify-end gap-1 ${(cryptoAssets[0].change24h ?? 0) >= 0 ? 'text-success' : 'text-destructive'}`}>
                      {(cryptoAssets[0].change24h ?? 0) >= 0 ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
                      <span className="text-sm md:text-base font-semibold">{cryptoAssets[0].changeText}</span>
                    </div>
                    <p className={`text-sm md:text-base font-semibold ${(cryptoAssets[0].change24h ?? 0) >= 0 ? 'text-success' : 'text-destructive'}`}>
                      {(cryptoAssets[0].change24h ?? 0) >= 0 ? '+' : '-'}${Math.abs(cryptoAssets[0].change24h ?? 0).toFixed(2)}
                    </p>
                  </div>
                </div>
              </Card>
                  ...
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 h-full">
                    <div className="flex items-center gap-2 md:gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-xs md:text-sm font-semibold truncate">{asset.symbol}/USDT</p>
                    ...
                    <div className="text-left md:text-right space-y-0.5 flex-shrink-0">
                      ...
                      <p className="text-[10px] md:text-xs text-muted-foreground">{getRelativeTime(asset.lastUpdated)}</p>
                    </div>
                  </div>
                </Card>
              ))
        ...
        <motion.div
          ...
          viewport={{ once: true, margin: "-100px" }}
        >
          <h3 className="text-xl md:text-2xl font-bold mb-4 md:mb-6">Ranking Asset by Market Cap</h3>
